...
export const initiatePayment = async (req, res) => {
  ...
  try {
    ...
    const insertQuery = ` INSERT INTO payments (business_id, plan, amount, transaction_id, status, created-at)
      VALUES (?, ?, ?, ?, ?, ?)`;
    db.execute(
      insertQuery,
...
export const verifyPayment = async (req, res) => {
    ...
    const updateQuery = `UPDATE payments SET transaction_id = ?, status = 'paid' WHERE transaction_id = ?`;
  
    db.execute(updateQuery, [razorpay_payment_id, razorpay_order_id], (err, result) => {
      if (err) {
